...
public class MainViewModel_SendTextMessage_Tests : MainViewModelTestBase
{
    ...
    public async Task SendTextMessage_OutGame_RoutesViaBrainEventRouter()
    {
        ...
        MockRouter.Verify(r => r.OnUserMessage(
            It.Is<ChatMessage>(m => m.Role == MessageRole.User && m.Content == "Test question")), Times.Once);
        MockRouter.Verify(r => r.OnAssistantMessage(
            It.Is<ChatMessage>(m => m.Role == MessageRole.Assistant && m.Content == "Mock brain reply for testing.")), Times.Once);
    }
    ...
    public async Task SendTextMessage_InGame_GetsContextAndSendsText()
    {
        ...
        // In-game chat now routes to brain service (SubmitQueryAsync), not voice
    ...
    public async Task SendTextMessage_InGame_SendFailure_SetsDeliveryFailed()
    {
        ...
        MockConversation.Setup(c => c.IsConnected).Returns(true);
        // In-game chat routes to brain, so failure comes from SubmitQueryAsync
        MockBrain.Setup(b => b.SubmitQueryAsync(It.IsAny<string>(), It.IsAny<SharedContextEnvelope>(), It.IsAny<CancellationToken>()))
...
public class MainViewModel_FrameCaptured_Tests : MainViewModelTestBase
{
    ...
    [Fact]
    public void FrameCaptured_StoreOnlyDecision_SkipsBrainButPersistsObservation()
    {
        MockObservationAdmissionGate.Setup(g => g.Evaluate(It.IsAny<bool>(), It.IsAny<DateTime>()))
            .Returns(new ObservationAdmissionDecision
            {
        ...
        MockObservationStore.Verify(s => s.StoreAsync(
            It.IsAny<ObservationWriteRequest>(), It.IsAny<CancellationToken>()), Times.Once);
        MockBrain.Verify(b => b.TrySubmitFrame(
    ...
    {
        // Frame has changed per diff service — TrySubmitFrame is called (not SubmitImageAsync)
        MockDiff.Setup(d => d.HasChanged(It.IsAny<byte[]>(), It.IsAny<int>(), It.IsAny<int>())).Returns(true);
        MockBrain.Setup(b => b.TrySubmitFrame(It.IsAny<byte[]>(), It.IsAny<string>())).Returns(true);
    ...
    [Fact]
    public void FrameCaptured_ChannelClosed_LogsDropped()
    {
        // When TrySubmitFrame returns false (channel closed / disposed), frame is logged as dropped
        MockDiff.Setup(d => d.HasChanged(It.IsAny<byte[]>(), It.IsAny<int>(), It.IsAny<int>())).Returns(true);
        MockBrain.Setup(b => b.TrySubmitFrame(It.IsAny<byte[]>(), It.IsAny<string>())).Returns(false);
        ...
        mockTelemetry.Verify(t => t.TrackEvent(
            "capture", "frame_dropped", It.Is<Dictionary<string, string>?>(d =>
                d != null && d["reason"] == "channel_closed")), Times.Once);
    }
    ...
    [Fact]
    public void FrameCaptured_UsesTrySubmitFrame_NotIsBusy()
    {
        // After 15-02 wiring change, FrameCaptured handler must call TrySubmitFrame
        // instead of checking IsBusy + calling SubmitImageAsync
        MockDiff.Setup(d => d.HasChanged(It.IsAny<byte[]>(), It.IsAny<int>(), It.IsAny<int>())).Returns(true);
        MockBrain.Setup(b => b.TrySubmitFrame(It.IsAny<byte[]>(), It.IsAny<string>())).Returns(true);
        ...
        // TrySubmitFrame should be called (new wiring)
        MockBrain.Verify(b => b.TrySubmitFrame(
            It.IsAny<byte[]>(), It.IsAny<string>()), Times.Once);

        // IsBusy should NOT have been accessed for gating (old pattern removed)
        // Note: IsBusy getter may still be accessed for other purposes (UI),
        // but SubmitImageAsync should NOT be called from the FrameCaptured handler
        MockBrain.Verify(b => b.SubmitImageAsync(
            It.IsAny<byte[]>(), It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never,
            "FrameCaptured should use TrySubmitFrame, not SubmitImageAsync");
    }

    [Fact]
    public void FrameCaptured_LogsTelemetry_WhenFrameSubmitted()
    {
        MockDiff.Setup(d => d.HasChanged(It.IsAny<byte[]>(), It.IsAny<int>(), It.IsAny<int>())).Returns(true);
        MockBrain.Setup(b => b.TrySubmitFrame(It.IsAny<byte[]>(), It.IsAny<string>())).Returns(true);
        ...
        mockTelemetry.Verify(t => t.TrackEvent(
            "capture", "frame_queued", It.IsAny<Dictionary<string, string>?>()), Times.Once);
    }

    [Fact]
    public void FrameCaptured_RejectedFrame_DoesNotUpdatePreviewImage()
    {
        MockObservationAdmissionGate.Setup(g => g.Evaluate(It.IsAny<bool>(), It.IsAny<DateTime>()))
            .Returns(new ObservationAdmissionDecision
            {
        ...
        sut.PreviewImage.Should().BeNull("rejected frames should not update PreviewImage");
    }

    [Fact]
    public void FrameCaptured_RejectedFrame_DoesNotRouteScreenCapture()
    {
        MockObservationAdmissionGate.Setup(g => g.Evaluate(It.IsAny<bool>(), It.IsAny<DateTime>()))
            .Returns(new ObservationAdmissionDecision
            {
        ...
        MockRouter.Verify(r => r.OnScreenCapture(
            It.IsAny<string>(), It.IsAny<TimeSpan>(), It.IsAny<string>()), Times.Never,
            "rejected frames should not create timeline checkpoints");
    }
}
...
{
    [Fact]
    public void MessageReceived_AddsAssistantMessage_WithoutLegacyBridge()
    {
        var sut = CreateSut();

        RaiseMessageReceived(new ChatMessage
        {
            Role = MessageRole.Assistant,
            Intent = MessageIntent.GeneralChat,
            Content = "Typed provider reply",
            Source = "Mock Provider"
        });

        sut.ChatMessages.Should().HaveCount(1);
        sut.ChatMessages[0].Role.Should().Be(MessageRole.Assistant);
        sut.ChatMessages[0].Content.Should().Be("Typed provider reply");
    }
    ...
    [Fact]
    public void TextReceived_BoardSensitiveWithoutFreshContext_AnnotatesBeforeUiInsertion()
    {
        MockVoiceGrounding = new Mock<IVoiceGroundingCoordinator>();
        MockVoiceGrounding.Setup(v => v.Evaluate("What is on e4?", true))
            .Returns(new VoiceGroundingDecision(
                VoiceTurnClass.BoardSensitive,
        ...
        RaiseTextReceived("What is on e4?");

        sut.ChatMessages.Should().HaveCount(1);
        sut.ChatMessages[0].Content.Should().Contain("⚠");
        sut.ChatMessages[0].Content.Should().Contain("[checking on that"); // Pack-driven grounding language (default fallback)
        sut.AiDisplayContent!.Text.Should().Contain("⚠");
        sut.SlidingPanelContent!.Text.Should().Contain("[checking on that");
        MockRouter.Verify(r => r.OnGeneralChat(It.Is<string>(s =>
            s.Contains("⚠") && s.Contains("[checking on that"))), Times.Once);
    }

    [Fact]
    public void TextReceived_BoardSensitiveWithoutFreshContext_SendsCorrectionInjection()
    {
        MockVoiceGrounding = new Mock<IVoiceGroundingCoordinator>();
        MockVoiceGrounding.Setup(v => v.Evaluate("Am I winning?", true))
            .Returns(new VoiceGroundingDecision(
                VoiceTurnClass.BoardSensitive,
                VoiceResponseMode.AcknowledgeUncertainty,
                HasFreshGroundedContext: false,
                GroundedSummary: null,
                Reason: "Grounded context is stale"));
        MockConversation.Setup(c => c.IsConnected).Returns(true);
        MockSession.Setup(s => s.Context).Returns(new SessionContext { State = SessionState.InGame });

        var sut = CreateSut();

        RaiseTextReceived("Am I winning?");

        MockConversation.Verify(c => c.SendContextualUpdateAsync(
            It.Is<string>(s => s.Contains("outdated") && s.Contains("not fully certain")),
            It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public void TextReceived_UnsolicitedVoice_EmitsGroundingTelemetry()
    {
        MockVoiceGrounding = new Mock<IVoiceGroundingCoordinator>();
        MockSessionTrace = new Mock<ISessionTraceService>();
        MockVoiceGrounding.Setup(v => v.Evaluate("What changed?", true))
            .Returns(new VoiceGroundingDecision(
                VoiceTurnClass.BoardSensitive,
        ...
        RaiseTextReceived("What changed?");

        MockSessionTrace.Verify(t => t.TrackEvent(
            "voice.grounding.evaluated",
            It.Is<Dictionary<string, string>>(p =>
                p["turn_class"] == "BoardSensitive" &&
                p["response_mode"] == "DeclineBoardCertainty" &&
                p["has_fresh_context"] == "False" &&
                p["reason"] == "No grounded context available")), Times.Once);
    }
}
...
public class MainViewModel_VoiceChatDecoupled_Tests : MainViewModelTestBase
{
    ...
    // ── Voice Chat Pending State (T15 Toggle Responsiveness) ──────────────

    [Fact]
    public void VoiceChatPending_DefaultsFalse()
    {
        var sut = CreateSut();
    ...
    [Fact]
    public void VoiceChatToggle_WhenNotConnected_NoPendingState()
    {
        var sut = CreateSut();
        sut.SelectedAgent = CreateTestAgent();
        sut.ConnectionState = ConnectionState.Disconnected;

        // Toggle ON when not connected — snaps back, no pending
        sut.IsVoiceChatActive = true;
    ...
    [Fact]
    public async Task VoiceChatToggle_WhenConnected_SetsPendingDuringStartup()
    {
        ...
        // Toggle ON — pending should be true while StartRecordingAsync runs
        sut.IsVoiceChatActive = true;

        // Give the fire-and-forget task a moment to start
        await Task.Delay(50);
        sut.IsVoiceChatPending.Should().BeTrue();
    ...
    [Fact]
    public async Task VoiceChatToggleOff_SetsPendingDuringShutdown()
    {
        var tcs = new TaskCompletionSource();
        MockAudio.Setup(a => a.StopRecordingAsync())
            .Returns(tcs.Task);
        var sut = CreateSut();
        sut.SelectedAgent = CreateTestAgent();
        sut.ConnectionState = ConnectionState.Connected;

        // Toggle ON first (mock StartRecordingAsync returns immediately by default)
        sut.IsVoiceChatActive = true;
        await Task.Delay(50);

        // Toggle OFF — pending during StopRecordingAsync
        sut.IsVoiceChatActive = false;
        await Task.Delay(50);
        sut.IsVoiceChatPending.Should().BeTrue();
    ...
    [Fact]
    public async Task VoiceChatToggle_StartFails_SnapsBackAndClearsPending()
    {
        MockAudio.Setup(a => a.StartRecordingAsync(It.IsAny<Action<byte[]>>()))
            .ThrowsAsync(new InvalidOperationException("Mic access denied"));
        var sut = CreateSut();
        sut.SelectedAgent = CreateTestAgent();
        sut.ConnectionState = ConnectionState.Connected;
        ...
        sut.IsVoiceChatActive.Should().BeFalse("toggle should snap back on failure");
        sut.IsVoiceChatPending.Should().BeFalse("pending should clear after failure");
    }

    [Fact]
    public async Task Disconnect_ClearsPendingState()
    {
        var tcs = new TaskCompletionSource();
        MockAudio.Setup(a => a.StartRecordingAsync(It.IsAny<Action<byte[]>>()))
            .Returns(tcs.Task);
        ...
        // Start voice chat (pending while starting)
        sut.IsVoiceChatActive = true;
        await Task.Delay(50);
        sut.IsVoiceChatPending.Should().BeTrue();
    ...
    public async Task SendTextMessage_OutGame_OnFailure_ShowsGhostCardWhenGhostActive()
    {
        ...
        MockGhost.Verify(g => g.ShowCard(
            FabCardVariant.Text, null, It.Is<string>(s => s.Contains("Chat failed")), null,
    ...
    public async Task SendTextMessage_OutGame_OnFailure_NoGhostCardWhenGhostInactive()
    {
        ...
        MockGhost.Verify(g => g.ShowCard(
            It.IsAny<FabCardVariant>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<string?>(),
