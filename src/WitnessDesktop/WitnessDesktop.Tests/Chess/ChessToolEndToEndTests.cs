...
namespace WitnessDesktop.Tests.Chess;
...
public class ChessToolEndToEndTests
{
    ...
    [Fact]
    public async Task AnalyzePositionEngine_StockfishNotReady_ReturnsStockfishNotReadyError()
    {
        // Stockfish NOT started — should return stockfish_not_ready error (no Lichess fallback)
        ...
        root.TryGetProperty("reason", out _).Should().BeTrue();
    }
