...
public class EndToEndTests
{
    ...
    [Fact]
    public async Task SettingsViewModel_InferenceModeRoundTrip_PersistsAndDiagnosticsReflect()
    {
        ...
        settings.InferenceMode.Should().Be(InferenceMode.LocalFirst, "mode persists to settings");

        // Refresh diagnostics — cloud-first UX prevents local/fallback messaging
        await vm.RefreshDiagnosticsAsync();
        vm.IsFallbackActive.Should().BeFalse("cloud-first UX: local modes not active in this build");
        vm.BrainProvider.Should().Be("Cloud");

        // Switch to CloudOnly
        vm.InferenceMode = InferenceMode.CloudOnly;
        await vm.RefreshDiagnosticsAsync();
        vm.IsFallbackActive.Should().BeFalse("CloudOnly mode has no fallback concept");
        vm.BrainProvider.Should().Be("Cloud");
    }
}
