...
public class PipelineIntegrationTests
{
    ...
    public async Task FrameDiff_ToBrain_ToChannel_ToRouter_EndToEnd()
    {
        ...
        // D-039: unstructured ImageAnalysis no longer creates timeline events (TopStrip only),
        // so poll L1 context ingestion instead.
        var deadline = DateTime.UtcNow.AddSeconds(5);
        SharedContextEnvelope? envelope = null;
        while (DateTime.UtcNow < deadline)
        {
            envelope = await contextService.GetContextForVoiceAsync(DateTime.UtcNow);
            if (envelope.ImmediateEvents.Any()) break;
            await Task.Delay(50);
        }

        // Assert: brain result should be ingested as L1 event
