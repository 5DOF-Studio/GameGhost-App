...
public class SettingsServiceTests
{
    ...
    [Fact]
    public void InferenceMode_Default_IsCloudOnly()
    {
        var svc = new SettingsService();
        svc.InferenceMode.Should().Be(InferenceMode.CloudOnly);
    }

    [Fact]
    public void InferenceMode_SetGet_RoundTrips()
    {
        var svc = new SettingsService();
        svc.InferenceMode = InferenceMode.LocalFirst;
        svc.InferenceMode.Should().Be(InferenceMode.LocalFirst);
    }

    [Fact]
    public void InferenceMode_Set_FiresSettingChangedEvent()
    {
        var svc = new SettingsService();
        string? changedName = null;
        svc.SettingChanged += (_, name) => changedName = name;

        svc.InferenceMode = InferenceMode.LocalOnly;
