...
namespace WitnessDesktop.Tests.FrameAnalysis;
...
public class FrameDiffServiceTests
{
    ...
    [Fact]
    public void GetDistanceFromLast_DoesNotMutateState()
    {
        _sut.HasChanged(ImageA, threshold: 1).Should().BeTrue();

        var distanceBefore = _sut.GetDistanceFromLast(ImageC);
        var distanceAgain = _sut.GetDistanceFromLast(ImageC);

        distanceBefore.Should().BeGreaterThan(0);
        distanceAgain.Should().Be(distanceBefore, "distance inspection should not mutate the cached last hash");
    }
