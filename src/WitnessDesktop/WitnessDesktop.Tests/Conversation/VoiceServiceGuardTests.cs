...
public class VoiceServiceGuardTests
{
    ...
    [Fact]
    public async Task Gemini_SendTextAsync_WhenDisconnected_RaisesErrorAndThrows()
    {
        using var service = new GeminiLiveService(EmptyConfig());
        string? errorMessage = null;
        service.ErrorOccurred += (_, msg) => errorMessage = msg;
        ...
        var act = () => service.SendTextAsync("test message");

        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage("*not connected*");
        errorMessage.Should().Contain("not connected");
    }
    ...
    [Fact]
    public async Task OpenAI_SendTextAsync_WhenDisconnected_RaisesErrorAndThrows()
    {
        using var service = new OpenAIRealtimeService(EmptyConfig());
        string? errorMessage = null;
        service.ErrorOccurred += (_, msg) => errorMessage = msg;
        ...
        var act = () => service.SendTextAsync("hello");
