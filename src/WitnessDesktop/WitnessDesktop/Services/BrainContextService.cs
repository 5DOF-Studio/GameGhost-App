...
public sealed class BrainContextService : IBrainContextService
{
    ...
    public BrainContextService(IVisualReelService visualReelService, ITelemetryService? telemetry = null, IExchangeManager? exchangeManager = null)
    {
    ...
    public string FormatAsPrefixedContextBlock(SharedContextEnvelope envelope)
    {
        ...
        if (!string.IsNullOrWhiteSpace(envelope.RecentVoiceTranscript))
        {
            sb.AppendLine("--- Recent voice ---");
            sb.AppendLine(envelope.RecentVoiceTranscript);
        }
        ...
        ContextAssemblyInputs? inputs)
    {
        ...
        // --- 3b. Voice transcript (newest first), truncate oldest when over budget ---
        var voiceTranscript = inputs?.RecentTranscript ?? [];
        var transcriptLines = new List<string>();
        foreach (var turn in voiceTranscript.Reverse().Take(30))
        {
            var role = turn.Role == TranscriptRole.User ? "User (voice)" : "AI (voice)";
            if (!string.IsNullOrWhiteSpace(turn.Text))
                transcriptLines.Add($"{role}: {turn.Text}");
        }

        budgetRemaining = budgetTokens - allocated;
        var transcriptIncluded = 0;
        var transcriptSummaryLines = new List<string>();
        for (var i = transcriptLines.Count - 1; i >= 0 && budgetRemaining > 0; i--)
        {
            var line = transcriptLines[i];
            var tok = EstimateTokens(line);
            if (tok <= budgetRemaining)
            {
                transcriptSummaryLines.Add(line);
                allocated += tok;
                budgetRemaining -= tok;
                transcriptIncluded++;
            }
        }
        if (transcriptIncluded < transcriptLines.Count && transcriptLines.Count > 0)
            report.Add($"transcript: {transcriptIncluded}/{transcriptLines.Count} turns");

        var recentVoiceTranscript = string.Join(Environment.NewLine, transcriptSummaryLines.AsEnumerable().Reverse());
        ...
        _telemetry?.TrackEvent("context", "envelope_built", new Dictionary<string, string>
        {
            ["l1Count"] = l1Included.Count.ToString(),
            ["l2Count"] = l2Events.Count.ToString(),
            ["budgetUsed"] = allocated.ToString(),
            ["budgetTotal"] = budgetTokens.ToString()
        });
        ...
        return new SharedContextEnvelope
        {
            ...
            RecentVoiceTranscript = recentVoiceTranscript,
            ExchangeContext = _exchangeManager != null ? new ExchangeContextInfo(
                _exchangeManager.CurrentExchange?.ExchangeId,
                _exchangeManager.IsExchangeActive,
                _exchangeManager.CurrentExchange != null
                    ? DateTime.UtcNow - _exchangeManager.CurrentExchange.OpenedAtUtc
                    : null,
                _exchangeManager.CurrentExchange?.AgentName) : null,
        };
    }
