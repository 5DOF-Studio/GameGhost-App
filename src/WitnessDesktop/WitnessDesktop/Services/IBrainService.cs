...
public interface IBrainService : IDisposable
{
    ...
    /// <summary>
    /// Submit a frame to the latest-frame-slot for async processing.
    /// Uses Channel(1, DropOldest) — always succeeds, never blocks.
    /// Returns false only if the channel is completed (service disposed).
    /// </summary>
    /// <param name="imageData">PNG image bytes from screen capture.</param>
    /// <param name="context">Current game state description for the analysis prompt.</param>
    bool TrySubmitFrame(byte[] imageData, string context);
