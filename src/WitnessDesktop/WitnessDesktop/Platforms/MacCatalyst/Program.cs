...
public class Program
{
	static void Main(string[] args)
	{
		...
		AppDomain.CurrentDomain.UnhandledException += (s, e) =>
		{
			...
			CrashLogger.LogCrash("AppDomain.UnhandledException (Program)", ex
				?? new Exception($"Non-exception: {e.ExceptionObject}"));
		};

		ObjCRuntime.Runtime.MarshalManagedException += (s, e) =>
		{
			Console.WriteLine($"[MARSHAL_MANAGED] {e.Exception}");
			CrashLogger.LogCrash("MarshalManagedException", e.Exception);
		};
