...
//
// Exports 18 @_cdecl functions consumed by .NET MAUI via DllImport/P/Invoke:
...
//   - ghost_panel_set_gear_tap_callback/set_audio_toggle_callback
//   - ghost_panel_set_audio_state/dismiss_audio_card
//
...
var gearTapCallback: (@convention(c) () -> Void)?
var audioToggleCallback: (@convention(c) (Int32, Bool) -> Void)?
...
public func ghostPanelCreate() -> Bool {
    ...
    runOnMainSync {
        ...
        // Default panel size -- covers a reasonable area for FAB + gear + cards
        let screenFrame = NSScreen.main?.visibleFrame ?? NSRect(x: 0, y: 0, width: 480, height: 600)
        let panelFrame = NSRect(
            x: screenFrame.maxX - 420,
            y: screenFrame.minY,
            width: 420,
...
/// Automatically repositions to the right edge of the current main screen
/// so the FAB is always visible.
...
public func ghostPanelShow() {
    ...
    DispatchQueue.main.async {
        ...
        // Reposition panel to right edge of current main screen
        let screenFrame = NSScreen.main?.visibleFrame ?? NSRect(x: 0, y: 0, width: 480, height: 600)
        let panelFrame = NSRect(
            x: screenFrame.maxX - 420,
            y: screenFrame.minY,
        ...
        NSLog("[GaimerGhostMode] Panel repositioned to: %@", NSStringFromRect(panelFrame))
...
/// Registers a C function pointer called when the user taps the gear icon.
@_cdecl("ghost_panel_set_gear_tap_callback")
public func ghostPanelSetGearTapCallback(callback: @convention(c) () -> Void) {
    NSLog("[GaimerGhostMode] ghost_panel_set_gear_tap_callback registered")

    gearTapCallback = callback

    DispatchQueue.main.async {
        sharedState?.onGearTap = {
            gearTapCallback?()
        }
    }
}

/// Registers a C function pointer called when an audio toggle changes.
/// Parameters: toggleIndex (0=MIC, 1=AUTO, 2=AI-MIC), newValue (bool).
...
public func ghostPanelSetAudioToggleCallback(callback: @convention(c) (Int32, Bool) -> Void) {
    NSLog("[GaimerGhostMode] ghost_panel_set_audio_toggle_callback registered")

    audioToggleCallback = callback

    DispatchQueue.main.async {
        sharedState?.onAudioToggle = { index, newValue in
            audioToggleCallback?(index, newValue)
        }
    }
}

/// Syncs audio toggle states from C# to the native audio card.
@_cdecl("ghost_panel_set_audio_state")
public func ghostPanelSetAudioState(micActive: Bool, commentaryActive: Bool, aiMicActive: Bool) {
    NSLog("[GaimerGhostMode] ghost_panel_set_audio_state: mic=%d, commentary=%d, aiMic=%d",
          micActive ? 1 : 0, commentaryActive ? 1 : 0, aiMicActive ? 1 : 0)

    DispatchQueue.main.async {
        sharedState?.isMicActive = micActive
        sharedState?.isCommentaryActive = commentaryActive
        sharedState?.isAiMicActive = aiMicActive
        (sharedPanel?.contentView as? GhostPanelContentView)?.audioStateDidChange()
    }
}

/// Dismisses the audio control card from C#.
@_cdecl("ghost_panel_dismiss_audio_card")
public func ghostPanelDismissAudioCard() {
    NSLog("[GaimerGhostMode] ghost_panel_dismiss_audio_card called")

    DispatchQueue.main.async {
        (sharedPanel?.contentView as? GhostPanelContentView)?.hideAudioCard(animated: true)
    }
}
