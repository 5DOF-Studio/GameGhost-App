...
public class MacGhostModeService : IGhostModeService, IDisposable
{
    ...
    public void SetAudioState(bool micActive, bool commentaryActive, bool aiMicActive)
    {
        if (!_isSupported) return;

        GhostModeNativeMethods.ghost_panel_set_audio_state(micActive, commentaryActive, aiMicActive);
    }
