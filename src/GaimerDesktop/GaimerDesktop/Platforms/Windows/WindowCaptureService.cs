...
using static GaimerDesktop.Platforms.Windows.NativeMethods;

namespace GaimerDesktop.Platforms.Windows;
...
public sealed class WindowCaptureService : IWindowCaptureService, IDisposable
{
    ...
    private const int DefaultCaptureIntervalMs = 1000; // 1 FPS
    ...
    public Task StartCaptureAsync(CaptureTarget target, int captureIntervalMs = 1000)
    {
        lock (_gate)
        {
            ...
            _captureTimer = new Timer(OnCaptureTimerTick, null, 0, _captureIntervalMs);
        }
