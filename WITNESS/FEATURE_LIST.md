...
- [ ] **Anchored overlay window (Windows-first)**
  ...
  - **Plan:** `WITNESS/gaimer_spec_docs/WINDOWS_OVERLAY_IMPLEMENTATION_PLAN.md`
...
- [✅] **Integration Verification** (Plan 06)
  ...
  - Design files: `WITNESS/gaimer_spec_docs/agents/{leroy,wasp}/`
...
- [✅] **Brain Improvements (Phase 14)** *(Completed March 10, 2026)*
  - GameJournalService maintains in-memory move journal across captures (no longer stateless)
  - Auto new-game detection via starting FEN comparison (resets journal + context)
  - BrainPromptBuilder with system/user message split, L1 event injection (capped at 5)
  - Vision prompt with identity + illusion language rules + tool awareness
  - In-game text chat routed to brain (full context: journal, tools, L1 events)
  - [ ] Parse window title for user's color (still pending)
  - [ ] FEN extraction → Stockfish validation (partially addressed by vision prompt guardrails)

### 16. Chess Context Engineering (Phase 14) — Complete
> **Plans:** `.planning/phases/14-chess-context-engineering/` (6 plans executed)
> **Completed:** March 10, 2026

- [✅] **Telemetry Infrastructure** (Plan 01)
  - ITelemetryService + ConsoleTelemetryService for structured pipeline observability
  - Optional null-default injection (zero impact on existing DI/tests)
  - 5 services instrumented (brain, capture, router, journal, context)

- [✅] **Lichess Fallback Removal** (Plan 02)
  - Dead Lichess code removed — Stockfish is sole chess engine
  - get_best_move returns unknown tool, stockfish_not_ready returns error JSON

- [✅] **Game Journal Service** (Plan 03)
  - In-memory move-by-move journal (200 cap, oldest evicted)
  - game_journal tool for voice agent access
  - BrainEventRouter ingests ImageAnalysis results into journal
  - FEN regex extraction from brain output

- [✅] **Brain Vision Prompt** (Plan 04)
  - Vision identity section shared across chess agents
  - Illusion language rules ("I can see the board")
  - Tool awareness (when to use Stockfish vs strategic analysis)

- [✅] **Brain System Prompt Architecture** (Plan 05)
  - BrainPromptBuilder with system/user message split
  - Context injection: L1 events (5 most recent), journal summary, rolling summary
  - Connection-state awareness (contextual, not imperative)

- [✅] **Integration Verification** (Plan 06)
  - Auto new-game detection (starting FEN, >2 entries threshold)
  - In-game text chat routed to brain SubmitQueryAsync
  - 648/648 total tests passing (119 new)

### 17. Vision Context Engineering (Phase 16) — Complete
> **Plans:** `.planning/phases/16-vision-context-engineering/` (5 plans executed)
> **Completed:** March 10, 2026

- [✅] **Brain Prompt Engineering** (Plan 01)
  - Chain-of-thought `visual_description` field for grounding observations before analysis
  - UNREADABLE escape hatch prevents confident confabulation on unclear positions
  - Confidence calibration tags: CERTAIN (>95%), LIKELY (75-95%), UNCERTAIN (50-75%), GUESSING (<50%)

- [✅] **Live Activity Bar Fix** (Plan 02)
  - TopStripUpdated event on IBrainEventRouter (event-based DI decoupling)
  - MainViewModel subscribes with MainThread.BeginInvokeOnMainThread dispatch
  - Fired at all 5 routing paths in BrainEventRouter

- [✅] **Gemini Model Switch** (Plan 03)
  - Brain model: anthropic/claude-sonnet-4 → google/gemini-2.5-flash
  - OpenRouter provider preferences: Google-only routing, allow_fallbacks: false
  - Worker model unchanged: openai/gpt-4o-mini

- [✅] **Structured JSON Output** (Plan 04)
  - BrainAnalysisResult model (7 nullable fields, ToDisplayText, ConfidenceScore)
  - response_format JSON schema with strict: true on OpenRouterClient
  - TryParseStructuredAnalysis in BrainEventRouter with free-text fallback
  - L1 event confidence extracted from structured response (was hardcoded 0.8)

- [✅] **Temporal Consistency Validation** (Plan 05)
  - ValidateTemporalConsistency on GameJournalService
  - Duplicate FEN detection + impossible piece count changes (>2 diff)
  - Telemetry tracking for temporal_inconsistency events
  - Entries still recorded (no data loss)

- [✅] **Code Review Fixes**
  - Triple-parsing eliminated (single structured variable reused)
  - JsonDocument leak fixed (using var on Parse() calls)
  - ConsoleTelemetryService: Console.WriteLine → Trace.WriteLine
  - 744/744 total tests passing (46 new)

### 18. Capture Pipeline Fix (Phase 15) — Complete
> **Plans:** `.planning/phases/15-capture-pipeline-fix/` (2 plans executed)
> **Completed:** March 10, 2026

- [✅] **Frame Slot Channel + Counter Fix** (Plan 01)
  - Fixed critical _activeTasks counter leak (Increment inside Task.Run delegate)
  - Channel(1, DropOldest) frame slot: TrySubmitFrame always succeeds, latest frame wins
  - ConsumeFramesAsync consumer loop: sequential processing, error recovery
  - Same counter fix in SubmitQueryAsync
  - Consumer loop uses parameterless ReadAllAsync() — survives CancelAll() (service is DI singleton)

- [✅] **MainViewModel Wiring + Telemetry** (Plan 02)
  - FrameCaptured handler rewired to TrySubmitFrame (IsBusy gate removed)
  - Frame lifecycle telemetry (queued, replaced, dropped, processing_start, error)
  - MockBrainService updated with matching Channel pattern + consumer task wait on Dispose
  - 698/698 total tests passing (50 new)
...
- [✅] **macOS Release Build** *(Completed March 9, 2026)*
  - AssemblyName changed from WitnessDesktop to Gaimer (bundle: `Gaimer.app`)
  - .NET 8.0 MacCatalyst target (arm64)
  - Apple Developer ID signing + notarization by Apple
  - Build script: `scripts/build-release-mac.sh` (publish → sign → notarize → staple → zip)
  - Release artifact: `Gaimer.app` (39MB zip)
  - Repo relocated from iCloud `~/Documents/` to `~/Developer/gAImer_desktop`
...
- [ ] **Audio Device Selection** (Research Complete - December 23, 2025)
  ...
  - **Research Document:** `WITNESS/AUDIO_DEVICE_SELECTION_RESEARCH.md`
