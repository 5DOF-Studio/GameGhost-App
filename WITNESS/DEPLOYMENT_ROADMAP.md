...
**Project:** Gaimer Desktop (.NET MAUI — macOS Catalyst + Windows)
**Created:** 2026-02-25 (Tuesday night)
**Original Deadline:** 2026-02-27 (Friday)
...
**macOS deployment is COMPLETE.** The app is notarized by Apple and distributable.

| Milestone | Status | Date |
|-----------|--------|------|
| macOS Release build (arm64) | Complete | Mar 9 |
| Apple notarization | Complete | Mar 9 |
| Build script (`scripts/build-release-mac.sh`) | Complete | Mar 9 |
| AssemblyName → Gaimer (bundle: `Gaimer.app`) | Complete | Mar 9 |
| Repo moved to `~/Developer/gAImer_desktop` | Complete | Mar 9 |
| Windows Release build | Pending | — |
| 519 tests passing, 39/39 plans complete | Complete | Mar 9 |

---

## Current Strategic Direction (March 19, 2026)

Deployment is no longer the only gating concern. Post-sprint live testing elevated a runtime-stability issue in the capture/brain path.

Current product and architecture direction:

- The app promise for recent gameplay memory should be implemented as a bounded local observation store, not as an in-memory-only reel and not as a stream of always-on brain calls.
- The hot window remains approximately five minutes. Older material should be summarized, virtualized, or discarded according to explicit retention policy.
- Storage direction: SQLite for metadata/index, filesystem for image/video artifacts under app-local storage.
- Capture direction: detect often, admit/store selectively, and escalate to brain only when novelty/salience justifies it.
- This architecture must support still images now and short video clips later under one observation schema.

Priority order after current stabilization bugs:

1. P0 live capture lag / stale-message flood containment
2. Observation-store seam and retention policy
3. Short-term brain working set and repeat suppression
4. Timeline/chat/ghost bounded retention (`T18`)
5. Generalist commentator agent (`T17`)

---

## Original Sprint Summary (Historical)
...
## Post-Sprint Architecture Priorities

These items supersede the old Friday sprint assumptions for current branch planning.

| # | Task | Status | Priority | Notes |
|---|------|--------|----------|-------|
| A1 | Observation store foundation | Planned | P0 | SQLite metadata/index + filesystem artifacts for recent gameplay memory |
| A2 | Capture admission / novelty gate | Planned | P0 | Cheap signature path before expensive preview/compress/brain fanout |
| A3 | Brain working set extension | Planned | P0 | App-owned short-term state: latest accepted visual state, recent insights, mode |
| A4 | Unified retention engine | Planned | P1 | Timeline/chat/ghost/reel retention aligned to five-minute hot window |
| A5 | Observation query APIs | Planned | P1 | Time-window and nearest-observation retrieval; future clip/video support |
| A6 | Semantic video lane | Deferred | P2 | Add short clip artifacts and video-capable worker without changing the observation-store contract |
...
| S5 | Audio device selection | Not Started | 2-3h | Stretch | Select input/output devices instead of system default. Research complete (`WITNESS/AUDIO_DEVICE_SELECTION_RESEARCH.md`). |
...
- M5: Check `WitnessDesktop.csproj` target frameworks; ensure `#if MACCATALYST` guards cover all native interop.
...
| Purpose | Path |
|---------|------|
| Solution | `src/WitnessDesktop/WitnessDesktop.sln` |
| Main project | `src/WitnessDesktop/WitnessDesktop/WitnessDesktop.csproj` |
| DI registration | `src/WitnessDesktop/WitnessDesktop/MauiProgram.cs` |
| Provider factory | `src/WitnessDesktop/WitnessDesktop/Services/Conversation/ConversationProviderFactory.cs` |
| Provider interface | `src/WitnessDesktop/WitnessDesktop/Services/Conversation/IConversationProvider.cs` |
| Provider impls | `src/WitnessDesktop/WitnessDesktop/Services/Conversation/Providers/` |
| Brain router | `src/WitnessDesktop/WitnessDesktop/Services/BrainEventRouter.cs` |
| Session manager | `src/WitnessDesktop/WitnessDesktop/Services/SessionManager.cs` |
| Timeline feed | `src/WitnessDesktop/WitnessDesktop/Services/TimelineFeed.cs` |
| Main ViewModel | `src/WitnessDesktop/WitnessDesktop/ViewModels/MainViewModel.cs` |
| Brain context | `src/WitnessDesktop/WitnessDesktop/Services/BrainContextService.cs` |
| Chat brain spec | `WITNESS/gaimer_spec_docs/chat-brain-design.md` |
| Implementation roadmap | `WITNESS/gaimer_spec_docs/implementation-roadmap.md` |
| Screen capture arch | `WITNESS/gaimer_spec_docs/SCREEN_CAPTURE_ARCHITECTURE.md` |

---

*Last updated: 2026-03-09. macOS notarized release complete. Windows build pending.*
