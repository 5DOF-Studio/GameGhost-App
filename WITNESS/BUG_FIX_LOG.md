...
### BUG-006: Voice provider selection ignored Settings and could route sessions to the wrong backend
- **ID:** BUG-006
- **Date Reported:** March 11, 2026
- **Date Fixed:** March 11, 2026
- **Severity:** High
- **Status:** ✅ Fixed
- **Component:** Conversation provider selection / voice session startup
- **Description:** Users could choose OpenAI or Gemini in Settings, but the live conversation provider factory still auto-selected based on API-key presence. When both keys existed, the app could connect to a different backend than the one shown in Settings.
- **Steps to Reproduce:**
  1. Store both Gemini and OpenAI API keys.
  2. Select `openai` in Settings.
  3. Start a voice session without `VOICE_PROVIDER` explicitly set in env.
- **Expected Behavior:** The session should use the provider selected in Settings.
- **Actual Behavior:** Factory preferred Gemini during auto-detect, ignoring the stored user preference.
- **Root Cause:** `ConversationProviderFactory.Create()` considered env overrides and key presence, but not `_settings.VoiceProvider` before auto-detect.
- **Fix:** Added a settings-preference branch before auto-detect so persisted `VoiceProvider` wins when the corresponding API key is available.
- **Files Changed:**
  - `src/WitnessDesktop/WitnessDesktop/Services/Conversation/ConversationProviderFactory.cs`
  - `src/WitnessDesktop/WitnessDesktop.Tests/Conversation/ConversationProviderFactoryTests.cs`
- **Tested:**
  - Added `Create_AutoDetect_BothKeys_UsesSettingsProviderPreference`
  - Verified with targeted net8.0 tests: 21/21 passing

### BUG-007: OpenAI Realtime voice session could capture mic audio without generating a response
- **ID:** BUG-007
- **Date Reported:** March 11, 2026
- **Date Fixed:** March 11, 2026
- **Severity:** Critical
- **Status:** ✅ Fixed
- **Component:** OpenAI Realtime voice session configuration
- **Description:** On connect, the microphone pipeline could start and audio chunks could be sent, but the user would not hear or see a response from the AI.
- **Steps to Reproduce:**
  1. Select OpenAI as the voice backend.
  2. Connect and enable voice chat.
  3. Speak into the mic and watch local mic activity.
- **Expected Behavior:** On speech stop, the session should create a response and stream back text/audio.
- **Actual Behavior:** Mic activity was present, but no audible response was produced.
- **Root Cause:** The `session.update` payload used `server_vad` but did not explicitly request automatic response creation/interruption behavior. This left turn handling underspecified for the current realtime session behavior.
- **Fix:** Introduced `OpenAiRealtimeSessionOptions.BuildSessionUpdateJson(...)` and added explicit `turn_detection.create_response = true` and `turn_detection.interrupt_response = true`. `OpenAIRealtimeService` now sends this canonical payload.
- **Files Changed:**
  - `src/WitnessDesktop/WitnessDesktop/Services/OpenAiRealtimeSessionOptions.cs`
  - `src/WitnessDesktop/WitnessDesktop/Services/OpenAIRealtimeService.cs`
  - `src/WitnessDesktop/WitnessDesktop.Tests/Conversation/OpenAiRealtimeSessionOptionsTests.cs`
- **Tested:**
  - Added `BuildSessionUpdateJson_EnablesServerVadAutoResponse`
  - Verified with targeted net8.0 tests: 21/21 passing

### BUG-008: Screen capture did not honor "every 30s or when image changes"
- **ID:** BUG-008
- **Date Reported:** March 11, 2026
- **Date Fixed:** March 11, 2026
- **Severity:** High
- **Status:** ✅ Fixed
- **Component:** Screen capture scheduling / emission
- **Description:** Product copy and agent prompts claimed capture happened every 30 seconds or when the image changed, but the runtime only emitted captures on a timer and applied diffing later in the brain submission path.
- **Steps to Reproduce:**
  1. Connect to a window with a 30-second capture interval.
  2. Change the board or game screen within the interval.
  3. Observe that no capture event is emitted immediately.
- **Expected Behavior:** First capture on connect, then a new frame whenever the image changes, or after the interval elapses.
- **Actual Behavior:** Capture emission depended on the timer; image changes inside the interval did not trigger earlier frame emission.
- **Root Cause:** The capture services were timer-driven only. `FrameDiffService` lived downstream in `MainViewModel`, so it could suppress brain submission, but it could not cause the capture services themselves to emit early when the image changed.
- **Fix:** Added `CaptureEmissionGate` and wired it into MacCatalyst, Windows, and mock capture services. Services now poll at up to 1 Hz and emit when the frame changes or the minimum interval elapses.
- **Files Changed:**
  - `src/WitnessDesktop/WitnessDesktop/Services/CaptureEmissionGate.cs`
  - `src/WitnessDesktop/WitnessDesktop/Platforms/MacCatalyst/WindowCaptureService.cs`
  - `src/WitnessDesktop/WitnessDesktop/Platforms/Windows/WindowCaptureService.cs`
  - `src/WitnessDesktop/WitnessDesktop/Services/MockWindowCaptureService.cs`
  - `src/WitnessDesktop/WitnessDesktop.Tests/Services/CaptureEmissionGateTests.cs`
- **Tested:**
  - Added `CaptureEmissionGateTests` covering first frame, unchanged-before-interval, changed-before-interval, and unchanged-after-interval
  - Verified with targeted net8.0 tests: 21/21 passing

### BUG-010: dHash 9x8 resolution too coarse for chess piece detection
- **ID:** BUG-010
- **Date Reported:** March 12, 2026
- **Date Fixed:** March 12, 2026
- **Severity:** Critical
- **Status:** ✅ Fixed
- **Component:** FrameDiffService / capture diff gate
- **Description:** Chess piece moves produce dHash distance=0 at 9x8 resolution (64-bit hash). The entire 4MB chess screenshot is reduced to 72 pixels before comparison, making individual piece movements invisible. Brain pipeline starved — only the first frame ever submitted.
- **Steps to Reproduce:**
  1. Connect to Apple Chess, start a game.
  2. Make several moves.
  3. Observe logs: `Frame unchanged (dHash distance=0, threshold=4)` for every capture after the first.
- **Expected Behavior:** Piece moves should produce meaningful dHash distance, triggering brain submission.
- **Actual Behavior:** Distance stays at 0-1 regardless of board changes. Brain receives only the initial frame.
- **Root Cause:** dHash algorithm resizes image to 9x8 grayscale (72 pixels). At this resolution, moving a single chess piece doesn't change the overall luminance gradient pattern. The hash is identical before and after a move.
- **Fix:** Added variable-width high-res dHash support. `CaptureConfig.DiffHashWidth` controls hash grid size per agent. Chess agents use 33 (33x32 = 1024-bit hash), default remains 9 (64-bit). At 33x32 resolution, individual board squares are distinguishable and piece moves produce distances of 4-23.
- **Files Changed:**
  - `src/WitnessDesktop/WitnessDesktop/Models/Agent.cs` — Added `DiffHashWidth` to `CaptureConfig`, set chess agents to 33
  - `src/WitnessDesktop/WitnessDesktop/Services/IFrameDiffService.cs` — Added `HasChanged(byte[], int, int)` and `GetDistanceFromLast(byte[], int)` overloads
  - `src/WitnessDesktop/WitnessDesktop/Services/FrameDiffService.cs` — Added `ComputeHighResHash`, `HammingDistance` for byte[] hashes, separate `_lastHighResHash` state
  - `src/WitnessDesktop/WitnessDesktop/ViewModels/MainViewModel.cs` — Passes `DiffHashWidth` from agent config
  - `src/WitnessDesktop/WitnessDesktop.Tests/ViewModels/MainViewModelTests.cs` — Updated mock setups for 3-arg HasChanged
- **Tested:**
  - Live test: move frames produce distance=4-23, static board stays 0-2
  - Brain submissions resumed after moves, tool calls (analyze_position_engine) firing
  - 776/776 tests passing

### BUG-011: CoT markers leak to timeline UI when structured JSON parse fails
- **ID:** BUG-011
- **Date Reported:** March 12, 2026
- **Severity:** Medium
- **Status:** Open
- **Component:** BrainEventRouter / timeline display
- **Description:** `**VISUAL DESCRIPTION:**` chain-of-thought markers appear in timeline event cards when brain returns raw text instead of structured JSON.
- **Root Cause:** `TryParseStructuredAnalysis()` returns null when brain output starts with `**VISUAL DESCRIPTION:**` instead of `{`. Fallback path `structured?.ToDisplayText() ?? result.AnalysisText` passes raw CoT text to UI without stripping markers. The brain model (google/gemini-2.5-flash via OpenRouter) does not reliably produce structured JSON despite `response_format` schema.
- **Fix (Proposed):** Two-part fix: 1) Presentation guard — strip CoT markers in the fallback path before display (immediate). 2) Real fix — investigate structured JSON compliance with current brain model, potentially switch to a model that respects `response_format` more reliably.

### BUG-012: Journal and router observability — Debug.WriteLine invisible in production
- **ID:** BUG-012
- **Date Reported:** March 12, 2026
- **Severity:** Low
- **Status:** Open
- **Component:** BrainEventRouter, GameJournalService, ConsoleTelemetryService
- **Description:** BrainEventRouter uses `Debug.WriteLine` throughout, GameJournalService logs via ITelemetryService which routes to `Trace.WriteLine` (ConsoleTelemetryService). Neither is visible in `/tmp/gaimer-debug.log` when running without a debugger. This makes it impossible to confirm journal entries, router behavior, or voice context injection during live testing.
- **Fix (Proposed):** Upgrade critical router/journal log lines to `Console.WriteLine` (same pattern applied to OpenRouterBrainService in this session).

### BUG-013: Gemini Live voice not operational — model/API issues
- **ID:** BUG-013
- **Date Reported:** March 12, 2026
- **Severity:** High
- **Status:** Open
- **Component:** GeminiLiveService
- **Description:** Gemini Live voice provider cannot connect. Original model `gemini-2.0-flash-live-001` shut down Dec 2025. Protocol rewritten (camelCase, new input schema, setupComplete wait) for `gemini-2.5-flash-preview-native-audio-dialog` but not yet verified against live API. Google API key in .env (`GEMINI_API_KEY`) may not have correct permissions for the native audio dialog model.
- **Workaround:** Use `VOICE_PROVIDER=openai` (working) until Gemini API access is resolved.

### BUG-014: Voice-brain grounding gap — voice mostly ungrounded despite brain activity
- **ID:** BUG-014
- **Date Reported:** March 12, 2026
- **Severity:** High
- **Status:** Open (investigation needed)
- **Component:** BrainEventRouter → voice context injection
- **Description:** Brain is analyzing frames and producing results (visible in logs and timeline). OpenAI voice is responding. But voice output appears ungrounded — hallucinating chess advice rather than using brain analysis. The voice-brain bridge (`SendContextualUpdateAsync` and `GetContextForVoiceAsync`) needs live verification to confirm brain context is reaching the voice provider.
- **Note:** Router logging is invisible (BUG-012), so we cannot yet confirm whether `SendContextualUpdateAsync` is being called with brain results.

### BUG-009: OPENROUTER_APIKEY shadows GEMINI_APIKEY — voice connects with wrong API key
- **ID:** BUG-009
- **Date Reported:** March 11, 2026
- **Date Fixed:** March 11, 2026
- **Severity:** Critical
- **Status:** Fixed
- **Component:** API key resolution / ConversationProviderFactory / GeminiLiveService
- **Description:** When both `GEMINI_APIKEY` and `OPENROUTER_APIKEY` are set, the Gemini voice session authenticates with the OpenRouter API key and silently fails — producing no audio response despite appearing connected.
- **Steps to Reproduce:**
  1. Set both `GEMINI_APIKEY` and `OPENROUTER_APIKEY` in environment.
  2. Launch app (auto-detect mode, no explicit `VOICE_PROVIDER`).
  3. Connect to a game, enable voice chat, speak.
- **Expected Behavior:** Voice session authenticates with `GEMINI_APIKEY` and produces audio responses.
- **Actual Behavior:** Total silence. Gemini WebSocket connects but session setup fails with wrong key.
- **Root Cause:** `MauiProgram.BuildConfiguration()` registers `AddEnvironmentVariables("GEMINI_")` and `AddEnvironmentVariables("OPENROUTER_")`. Both strip their prefix, mapping `GEMINI_APIKEY` and `OPENROUTER_APIKEY` to the same `configuration["APIKEY"]` slot. .NET configuration is last-wins, so `OPENROUTER_` (registered after `GEMINI_`) overwrites the Gemini value. Both `ConversationProviderFactory.GetGeminiApiKey()` and `GeminiLiveService` constructor read `configuration["APIKEY"]` first, picking up the wrong key.
- **Fix:** Removed the shared `APIKEY`/`API_KEY` slots from Gemini key resolution entirely. Both `GetGeminiApiKey()` and `GeminiLiveService` now only read explicit keys: `GeminiApiKey`, `GEMINI_APIKEY`, `GEMINI_API_KEY`. The polluted shared slot is never consulted.
- **Files Changed:**
  - `src/WitnessDesktop/WitnessDesktop/Services/Conversation/ConversationProviderFactory.cs`
  - `src/WitnessDesktop/WitnessDesktop/Services/GeminiLiveService.cs`
  - `src/WitnessDesktop/WitnessDesktop.Tests/Conversation/ApiKeyCollisionTests.cs` (new — 6 tests)
  - `src/WitnessDesktop/WitnessDesktop.Tests/Conversation/ConversationProviderFactoryTests.cs` (3 new tests)
- **Tested:**
  - `SharedApiKeySlot_IsPollutedByOpenRouter` — proves collision with real `AddEnvironmentVariables` layering
  - `Factory_WithCollision_StillSelectsGemini` — factory finds real key despite pollution
  - `Factory_OnlyOpenRouterKey_FallsToMock` — polluted slot alone doesn't create Gemini provider
  - Full suite: 760/760 passing
- **Working Copy Reference:** Compared against `/Users/tonynlemadim/Documents/5DOF Projects/GameGhost-App` (last known working build) which does NOT have `AddEnvironmentVariables("OPENROUTER_")`.
...
- **Files Changed:**
  - `src/WitnessDesktop/WitnessDesktop/App.xaml.cs`
  - `src/WitnessDesktop/WitnessDesktop/ViewModels/MainViewModel.cs`
...
- **Files Changed:**
  - `src/WitnessDesktop/WitnessDesktop/Platforms/MacCatalyst/AudioService.cs` — Added AVAudioSession configuration, activation, engine preparation, and interruption handling
...
- **Dec 14, 2025 — Concrete root cause from logs (why: stop guessing)**
  ...
  - Decision: tap the input node's **native format**, then convert/resample to `PCM16/16k/mono` before sending to Gemini.
    - Implemented in `src/WitnessDesktop/WitnessDesktop/Platforms/MacCatalyst/AudioService.cs`:
