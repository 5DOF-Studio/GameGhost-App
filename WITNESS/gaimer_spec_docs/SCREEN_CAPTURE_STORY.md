...
> "WitnessDesktop would like to record this computer's screen."
> [Don't Allow] [OK]
...
The user navigated to System Settings, found WitnessDesktop in the Screen Recording list, toggled it on, restarted the app. TCC still denied. Error -3801 persisted.
...
```bash
codesign --force --deep --sign - /tmp/WitnessDesktop.app
```
...
ditto --norsrc .../WitnessDesktop.app /tmp/WitnessDesktop.app
codesign --force --deep --sign "Apple Development: Ike Nlemadim (DCRQMPF7A9)" /tmp/WitnessDesktop.app
```
...
codesign --force --deep \
    --sign "Apple Development: Ike Nlemadim (DCRQMPF7A9)" \
    --entitlements /tmp/WitnessDesktop.entitlements \
    /tmp/WitnessDesktop.app
```

### Problem 7b: Deployed to /tmp/

We had been deploying to `/tmp/WitnessDesktop.app` to avoid the iCloud extended attributes issue. But macOS System Settings could not properly display an app running from `/tmp/`. In the Screen Recording permission list, the app showed up as `(null)` — no name, no icon. Some users reported that TCC refused to grant permissions to apps in non-standard locations entirely.

Fix: deploy to `/Applications/` instead:

```bash
rm -rf /Applications/WitnessDesktop.app
ditto --norsrc .../WitnessDesktop.app /Applications/WitnessDesktop.app
codesign ... /Applications/WitnessDesktop.app
```

### Problem 7c: Stale TCC entries

All the previous failed attempts — ad-hoc builds, unsigned builds, builds in different locations — had left a trail of conflicting entries in the TCC database. macOS was confused about which "WitnessDesktop" had which permissions.
...
dotnet build src/WitnessDesktop/WitnessDesktop/WitnessDesktop.csproj \
...
rm -rf /Applications/WitnessDesktop.app
ditto --norsrc \
    src/WitnessDesktop/WitnessDesktop/bin/Debug/net8.0-maccatalyst/maccatalyst-x64/WitnessDesktop.app \
    /Applications/WitnessDesktop.app
