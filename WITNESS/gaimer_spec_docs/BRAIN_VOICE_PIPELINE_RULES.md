...
```
Screen Capture (on change or on-demand)
         │
         │  JPEG/PNG bytes
         ▼
┌─────────────────────────────────────────┐
...
### Voice may also receive structured metadata

The no-raw-image rule does **not** mean voice should be limited to untyped plain text forever.

Voice may receive app-owned structured metadata derived from brain output, for example:

- freshness
- confidence
- urgency
- semantic class
- suggested speech mode

This metadata is the correct way to ground voice in facts without violating the architectural boundary.
...
### Semantic classes are allowed, but not required everywhere

The brain may produce multiple message classes from one analysis pass:
...
These classes should remain available downstream as metadata so voice, ghost mode, and future audio-intelligence systems can make better delivery decisions.

However, the immediate stabilization work does not require a full spoken rendering system for every class.
The near-term requirement is simpler:

- voice must avoid ungrounded certainty
- board-sensitive speech must use fresh grounded brain context
- class-aware speech styling may be added in a later audio-intelligence rotation
...
### Change-Detection Precept (Default)
1 Hz polling with change-only emission via `CaptureEmissionGate`:
```
Poll tick (1s) → capture → FNV hash compare → if changed: FrameCaptured → TrySubmitFrame → Brain
```
First frame always emits. Subsequent frames emit only when visual content differs from the last emission.
Unchanged boards are never resent — the brain retains its last analysis.
...
### 7.4 Voice invents board facts without grounded context
```csharp
// WRONG — provider text alone is not enough for factual board claims
if (userAskedAboutBoard)
{
    Speak(providerReply);
}
```
Board-sensitive voice output must be gated on grounded brain context, or explicitly framed as uncertain/stale.
...
### Scenario A: Board change detected during gameplay
```
1. Opponent makes a move
2. Poll tick (within 1s) captures frame, FNV hash differs from last emission
3. CaptureEmissionGate emits → FrameCaptured fires
4. TrySubmitFrame → Channel frame slot → Brain picks up frame
...
### Scenario D: Nothing changed (Change-Detection Precept)
```
1. Poll tick (1s) captures frame
2. CaptureEmissionGate: FNV hash matches last emission → no emit
3. Frame discarded — brain NOT called
4. No API cost, no timeline noise
```
