...
`NativeLibrary.SetDllImportResolver` can only be called **once per assembly**. The WitnessDesktop assembly has two native frameworks (GaimerScreenCapture and GaimerGhostMode), so a single resolver must handle both.
...
```
src/WitnessDesktop/NativeHelpers/GaimerGhostMode/build-xcframework.sh
```
...
After `dotnet build`, the app bundle at `bin/Debug/.../WitnessDesktop.app` must be codesigned with the `--deep` flag to cover all nested frameworks:
...
```bash
ls -la /Applications/WitnessDesktop.app/Contents/Frameworks/GaimerGhostMode.framework/
```

Check platform tag:
```bash
vtool -show-build /Applications/WitnessDesktop.app/Contents/Frameworks/GaimerGhostMode.framework/GaimerGhostMode
```

Check exported symbols:
```bash
nm -gU /Applications/WitnessDesktop.app/Contents/Frameworks/GaimerGhostMode.framework/GaimerGhostMode | grep ghost_panel
```

Check architecture:
```bash
file /Applications/WitnessDesktop.app/Contents/Frameworks/GaimerGhostMode.framework/GaimerGhostMode
```
...
See also: `WITNESS/gaimer_spec_docs/WINDOWS_OVERLAY_IMPLEMENTATION_PLAN.md` for additional Windows overlay design notes.
...
| `WitnessDesktop.csproj` | MSBuild | CopyGhostModeFramework target |
