...
- **UI mockup setup**
  - From `WITNESS/gaimer_spec_docs/ui_mockup`:
...
- **Solution and project structure**
  - Create MAUI solution: `WitnessDesktop.sln`.
  - Add primary MAUI project: `src/WitnessDesktop/`.
  - Add optional shared project: `src/WitnessDesktop.Core/` for constants/configuration.
...
- **Specs and documentation inclusion**
  - Ensure the following are included and tracked under `WITNESS/`:
...
- `WitnessDesktop.sln` and core project(s) exist and build successfully.
...
- `WITNESS/GAIMER_DESKTOP_APP_SPEC.md`  
  - Canonical platform-level spec. Only updated if architecture or cross-app behavior changes.

- `WITNESS/gaimer_spec_docs/gaimer_design_spec.md`  
  - Gaimer-specific UX and product specification.

- `WITNESS/gaimer_spec_docs/GAIMER_IMPLEMENTATION_PLAN_STAGE1-3.md`  
...
- **Ad-hoc notes (if needed)**
  - If any deviations from spec occur, note them briefly in:
    - Either a new `NOTES.md` under `WITNESS/gaimer_spec_docs/` (if needed later), or
