...
> Implemented in `gAImer_desktop/src/WitnessDesktop/WitnessDesktop/`
...
- [ ] Guided local setup UX modeled after the Stockfish onboarding flow
  - detect whether Ollama is installed
  - detect whether Ollama is running
  - detect whether the configured MiniCPM model is present
  - present a one-click local setup path instead of terminal-only instructions
  - pull model assets into the runtime store, not into the `.app` bundle
  - surface explicit recovery guidance when installation must be completed outside the app
...
**Packaging decision:** Do not ship the MiniCPM model inside the desktop app bundle. Use an app-managed first-run setup flow, similar to the Stockfish engine experience, so runtime and model assets can be installed, updated, and repaired independently of app releases.
