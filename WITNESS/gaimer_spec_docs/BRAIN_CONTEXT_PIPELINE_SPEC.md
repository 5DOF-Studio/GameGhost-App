...
- Token budget split strategy between RecentChatSummary and RecentVoiceTranscript (need live data).
- Whether voice transcript should be summarized before envelope injection or kept as raw turns.

---

## 15) Voice Transcript Bridge (Added 2026-03-27)

Gap identified: brain had no awareness of what the user said to the voice agent. Voice transcripts were logged to console but not stored or fed into the context envelope.

**Resolution:** `IVoiceTranscriptStore` ring buffer (5-min retention) captures user + AI voice turns from `IConversationProvider.UserTranscriptReceived`. `ContextAssemblyInputs.RecentTranscript` feeds turns into `BrainContextService.BuildEnvelope()` which budget-fits them as `SharedContextEnvelope.RecentVoiceTranscript`.

**Plan:** `docs/superpowers/plans/2026-03-27-voice-transcript-bridge-plan.md`

**Deferred to Phase 12:** Exchange-aware transcript windowing, transcript summarization, budget split tuning.
